...
def _gen_grille(rng):
    return sorted(rng.sample(range(1, 61), 8))
...
def _draw_ticket(c, serie, color_hex, rng):
    ...
    c.drawCentredString(CARD_X + CARD_W/2, hdr_y + 2.5*mm, "JEUX 60   8 BOULES   BY 2KEA")
...
def generate_pdf(nb_tickets=500, serie_start=1, output_path=None, game_name="60BOULES"):
    _register_font()
    if output_path is None:
        os.makedirs('/data', exist_ok=True)
        output_path = f'/data/{game_name}_{serie_start:05d}.pdf'
    c = canvas.Canvas(output_path, pagesize=(PAGE_W, PAGE_H))
    rng = random.Random(860000 + int(serie_start))
...
if __name__ == '__main__':
    path = generate_pdf(nb_tickets=12, serie_start=1, output_path='/mnt/user-data/outputs/60_BOULES_TEST.pdf')
    print(f"PDF : {path}")
