...
def joueuses_lesees():
    """ADMIN — Vraies lesees, en deduisant AUSSI les transferts envoyes, et en
    montrant ou sont partis les pions (circuit=victime / ailleurs=donne). Exclut
    le circuit. Lecture seule. ?cle=ADMIN"""
    global DB
    DB = load_data()
    cle = (request.args.get("cle", "") or "").strip().upper()
    info = DB.get("codes", {}).get(cle)
    ...
    while frontier and niveau<=6:
        ...
        for t in transferts:
            ...
            if de in frontier and vers and vers not in circuit:
                circuit.add(vers); suiv.add(vers)
        frontier=suiv
    env_circuit={}; env_ailleurs={}
    for t in transferts:
        de=(t.get("de") or "").upper(); vers=(t.get("vers") or "").upper(); m=int(t.get("montant",0) or 0)
        if not de: continue
        if vers in circuit: env_circuit[de]=env_circuit.get(de,0)+m
        else: env_ailleurs[de]=env_ailleurs.get(de,0)+m
    codes = set(achats)|set(gains)|set(tickets_dep)|set(retraits)|set(bc)
    lignes=[]; total_rembourser=0
    for c in codes:
        c=c.upper()
        if c in staff or c in circuit: continue
        vrai_entre = achats.get(c,0)+gains.get(c,0)+bc.get(c,0)
        sorties = tickets_dep.get(c,0)+retraits.get(c,0)+envoye.get(c,0)
        du_net = max(0, vrai_entre - sorties)
        actuel = solde(c)
        manque = max(0, du_net - actuel)
        ec = env_circuit.get(c,0); ea = env_ailleurs.get(c,0)
        if manque>0 or ec>0:
            lignes.append((c, noms.get(c,""), vrai_entre, ec, ea, manque, actuel))
    lignes.sort(key=lambda x:-((x[3])+(x[5])))
    rows=""
    for c,nom,ve,ec,ea,manque,act in lignes:
        if ec>0 and manque>0: statut="🟠 victime + reduite"; coul="#fb923c"; total_rembourser+=ec+manque
        elif ec>0: statut="🟢 victime (envoi circuit)"; coul="#34d399"; total_rembourser+=ec
        elif manque>0 and ea>0: statut="🔴 a donne ailleurs"; coul="#f87171"
        elif manque>0: statut="🟡 reduite par nettoyage"; coul="#fbbf24"; total_rembourser+=manque
        else: statut="—"; coul="#6e7681"
        rows+=('<tr style="border-bottom:1px solid rgba(255,255,255,.06)">'
               '<td style="padding:6px;font-family:monospace;color:#a78bfa">'+c+'</td>'
               '<td style="padding:6px;color:#94a3b8;font-size:12px">'+nom+'</td>'
               '<td style="padding:6px;text-align:right;color:#6ee7b7">'+format(ve,",")+'</td>'
               '<td style="padding:6px;text-align:right;color:#34d399">'+format(ec,",")+'</td>'
               '<td style="padding:6px;text-align:right;color:#f87171">'+format(ea,",")+'</td>'
               '<td style="padding:6px;text-align:right;color:#fbbf24;font-weight:700">'+format(manque,",")+'</td>'
               '<td style="padding:6px;color:'+coul+';font-size:12px;font-weight:600">'+statut+'</td></tr>')
    if not rows:
        rows='<tr><td colspan="7" style="padding:16px;text-align:center;color:#34d399">Aucune joueuse lesee.</td></tr>'
    html = ('<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Lesees</title></head>'
            '<body style="margin:0;background:#0f0e1f;font-family:system-ui,sans-serif;padding:16px;color:#fff"><div style="max-width:960px;margin:0 auto">'
            '<h1 style="font-size:20px;color:#fbbf24;margin-bottom:8px">🤝 Vraies lesees — qui a perdu quoi, et ou c\'est parti</h1>'
            '<div style="background:rgba(52,211,153,.1);border:1px solid #10b981;border-radius:10px;padding:12px;margin-bottom:12px;color:#6ee7b7">A rembourser (victimes du circuit + reduites par le nettoyage) : <b>'+format(total_rembourser,",")+' XPF</b>.</div>'
            '<div style="color:#94a3b8;font-size:12px;margin-bottom:10px">« → circuit » = pions partis vers la fraude (victime). « → ailleurs » = donne volontairement (pas du). « Manque » = du net − actuel APRES deduction des transferts (reduit par le nettoyage).</div>'
            '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:13px;min-width:760px">'
            '<tr style="border-bottom:2px solid #92400e;text-align:left"><th style="padding:6px;color:#fde68a">Code</th><th style="padding:6px;color:#fde68a">Nom</th><th style="padding:6px;color:#fde68a;text-align:right">Vrai entre</th><th style="padding:6px;color:#fde68a;text-align:right">→ circuit</th><th style="padding:6px;color:#fde68a;text-align:right">→ ailleurs</th><th style="padding:6px;color:#fde68a;text-align:right">Manque</th><th style="padding:6px;color:#fde68a">Statut</th></tr>'
            +rows+'</table></div></div></body></html>')
    return Response(html, mimetype="text/html; charset=utf-8")


@app.route("/rembourser-joueuse")
def rembourser_joueuse():
    """ADMIN — Rembourse UNE joueuse lésée, plafonné à son dû légitime réel
    (jamais plus que ce qu'elle a vraiment payé/gagné). Aperçu ; rien sans
    ...
    if not (info and info.get("admin")):
        return Response("Acces reserve. Ajoute ?cle=TON_CODE_ADMIN.", status=403, mimetype="text/plain; charset=utf-8")
    code = (request.args.get("code", "") or "").strip().upper()
    try: montant_dem = max(0, int(request.args.get("montant", "0") or "0"))
    except Exception: montant_dem = 0
    confirme = request.args.get("confirme", "") == "1"
    if not code:
    ...
    # Sources légitimes du code
    achats=gains=bonus=tickets_d=retraits=0
    for c in DB.get("commandes_pions_joueurs", []):
        if isinstance(c, dict) and (c.get("code_joueur") or "").upper()==code and c.get("statut")=="validee":
            nbp=int(c.get("nb_pions",c.get("pions_credites",0)) or 0); vp=int(c.get("valeur_pion",0) or 0)
            achats+=((nbp*vp) or int(c.get("montant_net",0) or 0))
    for g in DB.get("gains_finaux", []):
    ...
    # transferts envoyés AILLEURS (hors circuit) = sorties volontaires -> déduites du dû
    transferts=[t for t in DB.get("transferts_pions", []) if isinstance(t, dict)]
    envoye_tot={}; recu_tot={}
    for t in transferts:
        de=(t.get("de") or "").upper(); vers=(t.get("vers") or "").upper(); m=int(t.get("montant",0) or 0)
        if de: envoye_tot[de]=envoye_tot.get(de,0)+m
        if vers: recu_tot[vers]=recu_tot.get(vers,0)+m
    # circuit
    ach2={};gn2={};bc2={}
    for c in DB.get("commandes_pions_joueurs", []):
        if isinstance(c, dict) and c.get("statut")=="validee" and c.get("code_joueur"):
            cj=(c.get("code_joueur") or "").upper(); nbp=int(c.get("nb_pions",c.get("pions_credites",0)) or 0); vp=int(c.get("valeur_pion",0) or 0)
            ach2[cj]=ach2.get(cj,0)+((nbp*vp) or int(c.get("montant_net",0) or 0))
    for g in DB.get("gains_finaux", []):
        if isinstance(g, dict) and g.get("code"):
            cj=(g.get("code") or "").upper(); gn2[cj]=gn2.get(cj,0)+int(g.get("montant_gain",g.get("montant_credite",0)) or 0)
    for c in DB.get("credits_admin", []):
        if isinstance(c, dict) and c.get("code_joueur"):
            cj=(c.get("code_joueur") or "").upper(); bc2[cj]=bc2.get(cj,0)+int(c.get("nb_pions",0) or 0)*int(c.get("valeur_pion",0) or 0)
    fab=set()
    for c in envoye_tot:
        eu=ach2.get(c,0)+gn2.get(c,0)+bc2.get(c,0)+recu_tot.get(c,0)
        if envoye_tot[c]-eu>0: fab.add(c)
    circuit=set(fab); fr=set(fab); niv=0
    while fr and niv<=6:
        niv+=1; sv=set()
        for t in transferts:
            de=(t.get("de") or "").upper(); vers=(t.get("vers") or "").upper()
            if de in fr and vers and vers not in circuit: circuit.add(vers); sv.add(vers)
        fr=sv
    env_ailleurs=0
    for t in transferts:
        if (t.get("de") or "").upper()==code and (t.get("vers") or "").upper() not in circuit:
            env_ailleurs+=int(t.get("montant",0) or 0)

    actuel = solde(code)
    cible_legitime = max(0, achats+gains+bonus - tickets_d - retraits - env_ailleurs)
    plafond = max(0, cible_legitime - actuel)
    montant_eff = min(montant_dem, plafond)
    dans_circuit = code in circuit

    if confirme and montant_eff > 0:
        DB.setdefault("credits_admin", []).insert(0, {
            "code_joueur": code, "nb_pions": montant_eff, "valeur_pion": 1,
            "motif": "Remboursement victime fraude", "par": cle,
            "date": datetime.datetime.now().isoformat()})
        poser_solde(code, actuel + montant_eff)
        DB.setdefault("remboursements", []).insert(0, {"id":secrets.token_hex(4).upper(),"code":code,"montant":montant_eff,"par":cle,"date":datetime.datetime.now().isoformat()})
        save_data(immediat=True)
        actuel2 = solde(code)
        return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
        ...
        <div style="font-size:18px;font-weight:800;margin-bottom:8px">✅ Remboursement effectué</div>
        <div style="color:#cbd5e1;font-size:14px">{code} {nom} a été remboursé de <b style="color:#6ee7b7">{format(montant_eff, ",")} XPF</b>.<br>Nouveau solde : <b>{format(actuel2, ",")} XPF</b>.<br>Tracé comme « Remboursement victime fraude » sur son relevé.</div>
        </div></div></body></html>''', mimetype="text/html; charset=utf-8")

    avert = ""
    if dans_circuit:
        avert += '<div style="background:rgba(248,81,73,.15);border:1px solid #f85149;border-radius:8px;padding:10px;margin-bottom:10px;color:#fca5a5">⚠️ ATTENTION : ce code fait partie du <b>circuit de fraude</b>. Vérifie bien avant de rembourser.</div>'
    if montant_dem > plafond:
        avert += f'<div style="background:rgba(251,191,36,.12);border:1px solid #f59e0b;border-radius:8px;padding:10px;margin-bottom:10px;color:#fde68a">Le montant demandé ({format(montant_dem, ",")}) dépasse le dû légitime. Plafonné à <b>{format(plafond, ",")} XPF</b>.</div>'

    lien_conf = f"/rembourser-joueuse?cle={cle}&code={code}&montant={montant_eff}&confirme=1"
    return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Rembourser</title></head>
    <body style="margin:0;background:#0f0e1f;font-family:system-ui,sans-serif;padding:16px;color:#fff"><div style="max-width:600px;margin:0 auto">
      <h1 style="font-size:19px;color:#fbbf24;margin-bottom:10px">💰 Rembourser {code} <span style="color:#94a3b8;font-size:14px">{nom}</span></h1>
      {avert}
      <div style="background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin-bottom:12px;font-size:14px">
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Vrai argent payé + gains + bonus</span><b style="color:#6ee7b7">{format(achats+gains+bonus, ",")}</b></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Dépenses réelles (tickets+retraits+dons)</span><b style="color:#fca5a5">{format(tickets_d+retraits+env_ailleurs, ",")}</b></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Dû légitime</span><b style="color:#93c5fd">{format(cible_legitime, ",")}</b></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Solde actuel</span><b style="color:#8b949e">{format(actuel, ",")}</b></div>
        <div style="display:flex;justify-content:space-between;padding:6px 0;border-top:1px solid #30363d;margin-top:4px"><span style="color:#e6edf3;font-weight:700">Remboursement max possible</span><b style="color:#34d399">{format(plafond, ",")} XPF</b></div>
      </div>
      <div style="background:rgba(52,211,153,.08);border:1px solid #10b981;border-radius:10px;padding:14px">
        <div style="color:#6ee7b7;font-weight:700;margin-bottom:8px">👁️ Aperçu : rembourser <b>{format(montant_eff, ",")} XPF</b> → nouveau solde {format(actuel+montant_eff, ",")}</div>
        {"<a href='"+lien_conf+"' style='display:inline-block;background:#10b981;color:#fff;padding:12px 20px;border-radius:10px;text-decoration:none;font-weight:700'>✅ Confirmer le remboursement</a>" if montant_eff>0 else "<div style='color:#fbbf24'>Rien à rembourser (montant=0 ou plafond atteint). Ajoute &montant=XXXX dans l'adresse.</div>"}
      </div>
...
@app.route("/poser-solde-joueur")
def poser_solde_joueur():
    """ADMIN — Remet le compteur réel de pions d'un joueur au montant indiqué (=
    son SOLDE ACTUEL du relevé), pour que relevé = solde réel utilisable. Aperçu ;
    rien sans &confirme=1. ?cle=ADMIN&code=X&montant=Y[&confirme=1]"""
    global DB
    DB = load_data()
    cle = (request.args.get("cle", "") or "").strip().upper()
    info = DB.get("codes", {}).get(cle)
    ...
    if confirme and montant >= 0:
        poser(code, montant)
        DB.setdefault("ajustements_solde", []).insert(0, {"id":secrets.token_hex(4).upper(),"code":code,"avant":actuel,"apres":montant,"par":cle,"date":datetime.datetime.now().isoformat()})
        save_data(immediat=True)
        return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
        ...
        <div style="background:rgba(16,185,129,.15);border:2px solid #10b981;border-radius:12px;padding:20px;color:#34d399">
        <div style="font-size:18px;font-weight:800;margin-bottom:8px">✅ Solde réel mis à jour</div>
        <div style="color:#cbd5e1;font-size:14px">{code} {nom} : solde réel {format(actuel, ",")} ➜ <b style="color:#6ee7b7">{format(montant, ",")} XPF</b>.<br>Elle peut maintenant jouer avec ce solde.{"<br>⚠️ Note : ce compte est encore BANNI (codes_bloques). Si elle doit jouer, débloque-la." if banni else ""}</div>
        </div></div></body></html>''', mimetype="text/html; charset=utf-8")

    lien_conf = f"/poser-solde-joueur?cle={cle}&code={code}&montant={montant}&confirme=1"
    return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Poser solde</title></head>
    <body style="margin:0;background:#0f0e1f;font-family:system-ui,sans-serif;padding:16px;color:#fff"><div style="max-width:560px;margin:0 auto">
      <h1 style="font-size:19px;color:#22d3ee;margin-bottom:10px">⚖️ Remettre le solde réel — {code} <span style="color:#94a3b8;font-size:14px">{nom}</span></h1>
      {"<div style='background:rgba(248,81,73,.12);border:1px solid #f85149;border-radius:8px;padding:10px;margin-bottom:10px;color:#fca5a5'>⚠️ Ce compte est encore BANNI. Pense à le débloquer si elle doit jouer.</div>" if banni else ""}
      <div style="background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin-bottom:12px;font-size:14px">
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Solde réel actuel (compteur)</span><b style="color:#8b949e">{format(actuel, ",")} XPF</b></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Nouveau solde réel</span><b style="color:#6ee7b7">{format(montant, ",")} XPF</b></div>
      </div>
      <div style="background:rgba(34,211,238,.08);border:1px solid #0891b2;border-radius:10px;padding:14px">
        <div style="color:#67e8f9;font-weight:700;margin-bottom:8px">👁️ Aperçu : poser le solde réel à {format(montant, ",")} XPF</div>
...
@app.route("/restituer-legitime")
def restituer_legitime():
    """ADMIN — Débloque un compte + retire des pions illégitimes (fabriqués) du
    relevé via une correction tracée + pose le solde légitime. Garde relevé =
    compteur. Aperçu ; rien sans &confirme=1.
    ?cle=ADMIN&code=X&legitime=Y&retirer=Z[&confirme=1]"""
    global DB
    DB = load_data()
    cle = (request.args.get("cle", "") or "").strip().upper()
    info = DB.get("codes", {}).get(cle)
    if not (info and info.get("admin")):
        return Response("Acces reserve. Ajoute ?cle=TON_CODE_ADMIN.", status=403, mimetype="text/plain; charset=utf-8")
    code = (request.args.get("code", "") or "").strip().upper()
    try: legitime = max(0, int(request.args.get("legitime", "0") or "0"))
    except Exception: legitime = 0
    try: retirer = max(0, int(request.args.get("retirer", "0") or "0"))
    except Exception: retirer = 0
    confirme = request.args.get("confirme", "") == "1"
    if not code or legitime <= 0:
        return Response("Ajoute &code=LE_CODE&legitime=XXXX[&retirer=YYY]", status=400, mimetype="text/plain; charset=utf-8")
    nom = ""
    for t in DB.get("tickets", []):
        if isinstance(t, dict) and (t.get("code_acheteur") or "").upper()==code and t.get("acheteur"):
            nom = t.get("acheteur"); break
    ...
    if confirme:
        ...
        if retirer > 0:
            DB.setdefault("corrections_pions", []).insert(0, {
                "code_joueur": code, "montant_retire": retirer,
                "motif": "Pions fabriqués retirés (reçu d'un fabricant)", "par": cle,
                "date": datetime.datetime.now().isoformat()})
        poser(code, legitime)
        DB.setdefault("restitutions_legitimes", []).insert(0, {"id":secrets.token_hex(4).upper(),"code":code,"legitime":legitime,"retire":retirer,"par":cle,"date":datetime.datetime.now().isoformat()})
        save_data(immediat=True)
        return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
        <body style="margin:0;background:#0f0e1f;font-family:system-ui,sans-serif;padding:24px;color:#fff"><div style="max-width:560px;margin:0 auto">
        <div style="background:rgba(16,185,129,.15);border:2px solid #10b981;border-radius:12px;padding:20px;color:#34d399">
        <div style="font-size:18px;font-weight:800;margin-bottom:8px">✅ {code} restituée</div>
        <div style="color:#cbd5e1;font-size:14px">{code} {nom} est <b>débloquée</b>.<br>Pions fabriqués retirés : −{format(retirer, ",")}<br><b style="color:#6ee7b7">Solde légitime : {format(legitime, ",")} XPF</b><br><br><a href="/releve-financier-joueur/{code}?cle={cle}" style="color:#58a6ff">voir son relevé</a></div>
        </div></div></body></html>''', mimetype="text/html; charset=utf-8")

    lien_conf = f"/restituer-legitime?cle={cle}&code={code}&legitime={legitime}&retirer={retirer}&confirme=1"
    return Response(f'''<!DOCTYPE html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Restituer légitime</title></head>
    <body style="margin:0;background:#0f0e1f;font-family:system-ui,sans-serif;padding:16px;color:#fff"><div style="max-width:580px;margin:0 auto">
      <h1 style="font-size:19px;color:#22d3ee;margin-bottom:10px">⚖️ Restituer le légitime — {code} <span style="color:#94a3b8;font-size:14px">{nom}</span></h1>
      <div style="background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin-bottom:12px;font-size:14px">
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Statut</span><b style="color:{"#f87171" if est_banni else "#34d399"}">{"🚫 bannie → sera débloquée" if est_banni else "✅ active"}</b></div>
        <div style="display:flex;justify-content:space-between;padding:4px 0"><span style="color:#8b949e">Pions fabriqués à retirer</span><b style="color:#fca5a5">−{format(retirer, ",")}</b></div>
        <div style="display:flex;justify-content:space-between;padding:6px 0;border-top:1px solid #30363d;margin-top:4px"><span style="color:#e6edf3;font-weight:700">Solde légitime final</span><b style="color:#34d399">{format(legitime, ",")} XPF</b></div>
      </div>
      <div style="background:rgba(34,211,238,.08);border:1px solid #0891b2;border-radius:10px;padding:14px">
        <div style="color:#67e8f9;font-weight:700;margin-bottom:8px">👁️ Aperçu : débloquer + retirer {format(retirer, ",")} fabriqués + solde {format(legitime, ",")}</div>
        <a href="{lien_conf}" style="display:inline-block;background:#0891b2;color:#fff;padding:12px 20px;border-radius:10px;text-decoration:none;font-weight:700">✅ Confirmer</a>
      </div>
    </div></body></html>''', mimetype="text/html; charset=utf-8")
